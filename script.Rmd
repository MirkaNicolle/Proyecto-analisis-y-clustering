...
### Analisis pre decreto
```{r}
matripre <- read.csv("./CSV/Ma2015_2016.csv")
summary(matripre)
```
Como podemos ver en el analisis de la base de datos antes del decreto, tenemos, en el aparatado de EDADHOM y EDADMUj, las cuales representan las edaddes del hombre y de la mujer respectivamente, al momento de casarse, se tiene un minimo de 14 años en el caso del hombre y 12 en el caso de la mujer. 


### Analisis post decreto 
```{r}
matripost <- read.csv("./CSV/matrimonios17a19.csv")
summary(matripost)
```
En esta analisis de la base de datos post decreto, se puede ver en las mismas variables mencionadas anteriormente EDADHOM y EDADMUJ, que la edad minima para el hombre es de 17 y la edad minima de la mujer es de 16, mas adelante se corrobora por medio de una grafica de tendencia si realmente despues del decreto se a respetado la restriccion del casamiento de menores de edad. 
...
boxplot(datosMatrimonio$AÑOREG, main = "Caja y Bigotes de registro de matrimonios por año (2011 - 2019)", xlab = "Registro por año")
```
...
ggplot(data = datosMatrimonio, aes(x = EDADHOM)) + 
  ...
  ggtitle("Histograma con curva normal teórica de las edades de los hombres (2011 - 2019)") + 
  theme_bw()
```  
...
boxplot(datosMatrimonio$EDADHOM, main = "Caja y Bigotes de edad de los hombres al casarse (2011 - 2019)", xlab = "edad")
```
...
ggplot(data = datosMatrimonio, aes(x = EDADMUJ)) + 
  ...
  ggtitle("Histograma con curva normal teórica de las edades de las mujeres (2011 - 2019)") + 
  theme_bw()
```  
...
boxplot(datosMatrimonio$EDADMUJ, main = "Caja y Bigotes de edad de las mujeres al casarse (2011 - 2019)", xlab = "edad")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Distribución de departamento de ocurrencia (2011 - 2019)", xlab = "Departamento de ocurrencia")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Distribución de municipio de ocurrencia (2011 - 2019)", xlab = "Municipio de ocurrencia")
```
...
barplot(counts, las = 1, cex.names=.8 ,main = "Distribución de escolaridad de los hombres (2011 - 2019)", xlab = "Escolaridad del hombre")
```
...
barplot(counts, las = 1, cex.names=.8 ,main = "Distribución de escolaridad de las mujeres (2011 - 2019)", xlab = "Escolaridad de las mujeres")
```
...
barplot(counts, las = 2, cex.names=.8 ,main = "Distribución de etnia de los hombres (2011 - 2019)", xlab = "Etnia del hombre")
```
...
barplot(counts, las = 2, cex.names=.8 ,main = "Distribución de etnia de las mujeres (2011 - 2019)", xlab = "Etnia de las mujeres")
```
...
plot(x = datosMatrimonio$EDADHOM, y= datosMatrimonio$EDADMUJ, xlab= "Edad del Hombre", ylab= "Edad de la mujer", main = "Correlación entre las edades de los cónyuges (2011 - 2019)")
...
plot(x = menorEdad$EDADHOM, y= menorEdad$EDADMUJ, xlab= "Edad del Hombre", ylab= "Edad de la mujer", main = "Correlación entre las edades, mujeres menores de 18 (2011 - 2019)")
...
barplot(counts, las = 2, cex.names=.6 ,main = "Departamentos con matrimonios con menores de edad (2011 - 2019)", xlab = "Departamentos de ocurrencia")
```
...
barplot(counts, las = 2, cex.names=.5 ,main = "Municipios con matrimonios con menores de edad (2011 - 2019)", xlab = "Municipios de ocurrencia")
```
...
barplot(counts, las = 2, cex.names=.8 ,main = "Etnia de mujeres involucradas en matrimonios con menores de edad (2011 - 2019)", xlab = "Etnia de las mujeres")
```
...
barplot(counts, las = 2, cex.names=.8 ,main = "Etnia de hombres involucrados en matrimonios con menores de edad (2011 - 2019)", xlab = "Etnia de los hombres")
```
...
## Ocurrencia de matrimonios con menores de edad desde 2011 a 2019
...
Puede observarse en el gráfico que el codo se encuentra en el valor 7, por lo que esta es la cantidad adecuada de clusters que se deben utilizar.

## Agrupamiento por medio de k-means  

```{r kmeans, echo=FALSE}
library(fpc)
km<-kmeans(datosClustering[,1:4],7,iter.max =500)
...
Se puede observar cómo los datos se encuentran divididos en 7 grupos, todos bastante concentrados a excepcion de algunos datos atipicos del grupo mas alejado, estos son alrededor de 40 matrimonios donde los hombres eran de edades de 80 - 60 con niñas menores de edad.

## Mixture of gaussians

```{r gaussians, echo = FALSE}
library(mclust)
mc<-Mclust(datosClustering[,1:4],7)
...
library(cluster)
set.seed(123)
fcmf<-cmeans(datosClustering[,1:4],7)
...
```
En el primer gráfico, clusplot, se puede ver detrás las regiones y como se encuentran los datos, además se indica que se explica casi 48% del punto de variabilidad.
